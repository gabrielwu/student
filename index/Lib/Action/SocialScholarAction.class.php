<?php
 class SocialScholarAction extends Action{
	...
	public function index(){
		...
		$scholar_name = $_GET['scholar_name'];
		$level = $_GET['level'];
		$amount = $_GET['amount'];
		$date = $_GET['date'];
		...
		$condition['scholar_name'] = $scholar_name;
		...
		if ($scholar_name != '') {
		    $sqlWhere .= "and ss.competition_name like '$scholar_name%' ";
		}
		if ($level != '') {
		...
		$sqlFrom = "from student s, social_scholar ss where ss.stu_id=s.stu_id ";
	...
	public function changeSocialScholar(){
		header("Content-Type:text/html; charset=utf-8");
		$this->checklogin();
		$model = M();
		$id = $_GET["id"];	
	...
	public function updateSocialScholar(){
		header("Content-Type:text/html; charset=utf-8");
		$this->checklogin();
		$model = M("social_scholar");
		$id = $_POST["id"];
		...
		if ($model->where("id=$id")->save($data)) {
			$this->redirect("SocialScholar/index", "",2,"数据修改成功！！！");
		} else {
			$this->redirect("SocialScholar/index", "",2,"数据修改失败！！！");
		}
	}
	public function deleteSocialScholar(){
		header("Content-Type:text/html; charset=utf-8");
		$this->checklogin();
		$model = M("social_scholar");
		$id = $_POST["id"];
		if ($model->where("id=$id")->delete()) {
			$this->redirect("SocialScholar/index", "",2,"数据删除成功！！！");
		} else {
			$this->redirect("SocialScholar/index", "",2,"数据删除失败！！！");
		}
	}
	...
	public function uploadFile(){
		...
		if(!$upload->upload("upload/social_scholar/")){
		...
		$uploadFile="upload/social_scholar/$filename";
		$objReader  = PHPExcel_IOFactory::createReader('Excel5');
		$objPHPExcel = $objReader->load($uploadFile); 
		$sheet = $objPHPExcel->getSheet(0); 
		$highestRow = $sheet->getHighestRow(); // 取得总行数 
		$socialScholar = M("social_scholar");
		$model = M();
		...
		$count = 0;
			...
			for ($i = 2; $i <= $highestRow; $i++) {
				...
				foreach ($student as &$s) {
				    if ($s['stu_num'] == $stu_num) {
				        ...
				        $data["scholar_name"] = $objPHPExcel->getActiveSheet()->getCell("D$i")->getValue();
			...
			$this->redirect("SocialScholar/index", "", 0.01);
	}
	...
	public  function downDataFile(){
		...
		$objAlignN6 = $objStyleN6->getAlignment();
		$objAlignN6->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objActSheet->setCellValue('D1',"社会奖学金名称");
		...
		for($i=0;$i<count($result);$i++){
			...
			$objActSheet->setCellValue('D'.$count,$result[$i]["scholar_name"]);
	...
	public function download(){
		...
		import("ORG.Net.Http");
			$time = time();
			Http::download("upload/tpl/social_scholar.xls","$time.xls");
	}
 }
