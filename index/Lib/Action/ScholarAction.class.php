<?php
 class ScholarAction extends Action{
	...
	public function index(){
		...
		$sqlFrom = "from student s, scholar ss where ss.stu_id=s.stu_id ";
	...
	public function changeScholar(){
		header("Content-Type:text/html; charset=utf-8");
		$this->checklogin();
		$model = M();
		$id = $_GET["id"];	
		$sqlSelect = "select ss.*, s.stu_num, s.stu_name, s.stu_gender ";
		$sqlFrom = "from student s, scholar ss where s.stu_id=ss.stu_id ";
	...
	public function updateScholar(){
		...
		if ($model->where("id=$id")->save($data)) {
			$this->redirect("Scholar/index", "",2,"数据修改成功！！！");
		} else {
			$this->redirect("Scholar/index", "",2,"数据修改失败！！！");
		}
	}
	public function deleteScholar(){
		...
		if ($model->where("id=$id")->delete()) {
			$this->redirect("Scholar/index", "",2,"数据删除成功！！！");
		} else {
			$this->redirect("Scholar/index", "",2,"数据删除失败！！！");
		}
	}
	...
	public function uploadFile(){
		...
		if(!$upload->upload("upload/Scholar/")){
		...
		$uploadFile="upload/Scholar/$filename";
		...
		$count = 0;
			...
			$this->redirect("Scholar/index", "", 0.01);
	}
	...
	public function download(){
		...
		import("ORG.Net.Http");
			$time = time();
			Http::download("upload/tpl/scholar.xls","$time.xls");
	}
 }
