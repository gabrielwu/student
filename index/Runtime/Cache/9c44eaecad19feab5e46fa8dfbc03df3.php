...
<script type="text/javascript">
 function updateGrant(id){
	 ...
	 var id = id;
	 $("#id").attr("value",id);
	 window.showModalDialog('<?php echo ($url); ?>/changeGrant/id/'+id,obj,'dialogWidth:400px;dialogHeight:350px');
 }
...
<DIV id=wrap>
  <DIV id=title class=tab>
    <H2>学生事务</H2>
    <UL id=tab>
      <LI class=current><A href="#" target="main" >助学金</A></LI>
      ...
      <table class="list" width="100%" cellpadding="0" cellspacing="0" >
        <tbody>
		  ...
		  <tr class="">
            ...
            <th><input type="text" style="width:90%" name="grant_name" value="<?php echo ($condition["grant_name"]); ?>"></th>
          ...
          <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): ++$i;$mod = ($i % 2 )?><tr class="">
            ...
            <td><?php echo ($list["grant_name"]); ?></td>
            <td><?php echo ($list["level"]); ?></td>
			<td><?php echo ($list["amount"]); ?></td>
            <td><?php echo ($list["date"]); ?></td>            
            <td align="center">
			    <button type="button" class="add" onclick="updateGrant(<?php echo ($list["id"]); ?>)" value="修改">修改</button>
			    <button type="button" class="reset" onclick="deleteGrant(<?php echo ($list["id"]); ?>)" value="删除">删除</button>
            </td>
