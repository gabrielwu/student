...
<script type="text/javascript">
 function updateCompetition(id){
	 ...
	 var id = id;
	 $("#id").attr("value",id);
	 window.showModalDialog('<?php echo ($url); ?>/changeCompetition/id/'+id,obj,'dialogWidth:400px;dialogHeight:350px');
 }
...
<DIV id=wrap>
  <DIV id=title class=tab>
    <H2>学生事务</H2>
    <UL id=tab>
      <LI class=current><A href="#" target="main" >大赛获奖信息</A></LI>
      <LI ><A href="<?php echo ($url); ?>/inputStream" target="main" >数据导入</A></LI>
    </UL>
  </DIV>
  <DIV id=content >
    <form action="<?php echo ($url); ?>/deleteCompetition" method="post" id="form">
      <table class="list" width="100%" cellpadding="0" cellspacing="0" >
        <tbody>
		  ...
		  <tr class="">
            ...
            <th><input type="text" style="width:90%" name="competition_name" value="<?php echo ($condition["competition_name"]); ?>"></th>
          ...
          <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): ++$i;$mod = ($i % 2 )?><tr class="">
            ...
            <td><?php echo ($list["competition_name"]); ?></td>
            <td><?php echo ($list["level"]); ?></td>
			<td><?php echo ($list["award"]); ?></td>
            <td><?php echo ($list["date"]); ?></td>            
            <td align="center">
			    <button type="button" class="add" onclick="updateCompetition(<?php echo ($list["id"]); ?>)" value="修改">修改</button>
				<button type="button" class="reset" onclick="deleteCompetition(<?php echo ($list["id"]); ?>)" value="删除">删除</button>
            </td>
